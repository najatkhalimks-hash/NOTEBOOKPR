...
import { saveToSupabase, loadAllFromSupabase, isSupabaseConfigured } from './supabase.js'
import {
...
export default function App({ lang = 'fr' }) {
  ...
  const [supabaseRows, setSupabaseRows] = useState(null)  // null = pas encore chargé
  const [supabaseLoading, setSupabaseLoading] = useState(false)

  // Charger les données Supabase quand l'admin se connecte
  useEffect(() => {
    if (adminOk && supabaseRows === null) {
      setSupabaseLoading(true)
      loadAllFromSupabase().then(rows => {
  ...
  async function saveChercheurAll() {
    ...
    if (form.grade === 'Postdoctorant') {
      ...
      if (total < 100) { showToast(lang==='fr' ? `Affectation incomplète : ${total}% sur 100% requis` : `Incomplete assignment: ${total}% of 100% required`, 'error'); return }
      if (total > 100) { showToast(lang==='fr' ? `Affectation dépasse 100% (${total}%)` : `Assignment exceeds 100% (${total}%)`, 'error'); return }
    }
    setLoading(true)
    ...
    // Envoi Supabase — toutes les données du profil + activités
    await saveToSupabase(email, form.nom, form.prenom, form.grade, form.axe_principal, form.annee_academique || annee, 'profil_chercheur', {
      ...form,
      publications: pubRows,
      enseignements: ensRows,
      encadrements: encRows,
      prestations: presRows,
      rayonnements: rayRows,
      affectations: affectRows,
    })
  ...
  async function saveAffilieAll() {
    ...
    const total = affectationTotal(affectRows)
    if (total > 100) { showToast(`Le total des affectations (${total}%) dépasse 100%`, 'error'); return }
    setLoading(true)
    ...
    // Envoi Supabase
    await saveToSupabase(email, form.nom, form.prenom, form.grade, form.axe, form.annee_academique || annee, 'profil_affilie', {
      ...form,
      publications: pubRows,
      enseignements: ensRows,
      encadrements: encRows,
  ...
  async function submitPrevision() {
    ...
    await saveToSupabase(myEmail, form.nom||'', form.prenom||'', form.grade||'', form.axe_principal||'', annee, 'prevision', form)
    showToast(t.prev_saved + annee)
    setView('thanks')
  }

  async function submitRevision() {
    if (!form.motif) { showToast(lang==='fr'?'Le motif de révision est obligatoire':'Revision reason is required','error'); return }
    addRevision(myEmail, annee, form)
    await saveToSupabase(myEmail, '', '', '', '', annee, 'revision', form)
  ...
  if (view === 'admin') {
    ...
    // Données Supabase (cloud) fusionnées avec localStorage
    // supabaseRows est chargé au montage via useEffect
    const allSupabase = supabaseRows || []
    const supChercheurs = allSupabase.filter(r => r.mode === 'profil_chercheur')
    const supAffilies   = allSupabase.filter(r => r.mode === 'profil_affilie')
    // Fusionner : Supabase en priorité, localStorage en fallback
    const mergedChercheurs = [
      ...supChercheurs.map(r => ({ ...r.payload, email: r.email, nom: r.nom, prenom: r.prenom, grade: r.grade, axe_principal: r.axe, _source: 'cloud' })),
      ...chercheurs.filter(c => !supChercheurs.find(s => s.email === c.email)),
    ]
    const mergedAffilies = [
      ...supAffilies.map(r => ({ ...r.payload, email: r.email, nom: r.nom, prenom: r.prenom, grade: r.grade, axe: r.axe, _source: 'cloud' })),
      ...affilies.filter(a => !supAffilies.find(s => s.email === a.email)),
    ]
    const allPersonnes = [...mergedChercheurs, ...mergedAffilies]

    const personnes = adminRole===ROLES.RESPONSABLE_AXE
      ? allPersonnes.filter(p => (p.axe_principal||p.axe)===adminAxe)
      : allPersonnes
    ...
    const alerts = computeAlertes({ chercheurs: mergedChercheurs, affilies: mergedAffilies, projets, publications })
      ...
      <div style={{ minHeight:'100vh', background:C.g1, fontFamily:'system-ui,-apple-system,sans-serif' }}>
        ...
        <div style={{ background:C.navy, padding:'14px 20px', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:10, position:'sticky', top:0, zIndex:10 }}>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
            ...
            <button onClick={()=>{ setSupabaseRows(null) }} style={{ background:C.teal, color:'#fff', border:'none', borderRadius:8, padding:'7px 12px', fontSize:11, cursor:'pointer' }}>
              {supabaseLoading ? '⟳ Chargement…' : isSupabaseConfigured() ? `☁ ${(supabaseRows||[]).length} réponses cloud` : '⚠ Local seulement'}
            </button>
          </div>
        </div>
